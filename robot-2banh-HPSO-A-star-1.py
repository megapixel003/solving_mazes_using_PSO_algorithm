...
class Robot:
    ...
    def __init__(self, startPos, robotImg, map):
        ...
        self.circle_penalty = 1000  # Giảm hình phạt
        ...
        self.no_progress_penalty = 1000  # Giảm hình phạt
...
if __name__ == "__main__":
    ...
    # Số lượng scout particles
    num_scouts = int(0.2 * pop_size)  # 20% số hạt là scout particles
    stagnation_threshold = 5  # Số lần lặp không cải thiện trước khi khám phá lại
    ...
    try:
        while running and current_iteration < max_iteration:
            ...
            while num_robot_available > 0 and run_time < max_run_time:
                for idx, robot in enumerate(robots):
                    if not (robot.crash or robot.is_circling or robot.no_progress or robot.is_timeout or robot.reached_goal):
                        ...
                        # Thưởng/phạt dựa trên khoảng cách từ vị trí xuất phát
                        start_distance = np.sqrt((robot.x - start[0])**2 + (robot.y - start[1])**2)
                        if start_distance > 50:  # Nếu robot cách xa vị trí xuất phát hơn 50 pixel
                            start_distance_reward = 500 / (start_distance + 1)  # Thưởng
                        else:  # Nếu robot ở gần vị trí xuất phát
                            start_distance_reward = -1000  # Phạt
                        ...
                        robot.cost_function = (
                                              ...
                                              1.0 * start_distance_reward  # Thêm thành phần thưởng/phạt này
                                            )
            ...
            # Tăng độ ngẫu nhiên cho scout particles
            for i in range(num_scouts):
                scout_idx = np.random.randint(0, pop_size)
                V_pso[scout_idx] += np.random.uniform(-1, 1, npar) * 2  # Tăng độ ngẫu nhiên

            # Nếu không cải thiện sau một số lần lặp, một số hạt tự khám phá lại
            if stagnation_counter >= stagnation_threshold:
                for i in range(int(0.3 * pop_size)):  # 30% số hạt tự khám phá lại
                    random_idx = np.random.randint(0, pop_size)
                    P_pso[random_idx] = np.random.uniform(min_max[0], min_max[1], npar)
                    V_pso[random_idx] = np.zeros(npar)  # Reset vận tốc
