...
def af(x):
    return np.tanh(x)
def vd_nn(X, W, V, b_hidden):
    # Tính toán lớp ẩn
    net_h = W.T @ X + b_hidden
    y_h = af(net_h)
...
class Robot:
    ...
    def __init__(self, startPos, robotImg, map):
        ...
        self.circle_threshold = 8 
        self.circle_penalty = 2000
        self.is_circling = False

        # Lịch sử khoảng cách để kiểm tra tiến triển
        self.distance_history = []
        self.distance_history_length = 1000
        self.progress_threshold = 0.07 # 0.05 -> 0.07
        self.no_progress_penalty = 800
        ...
        self.timeout_penalty = 500
        self.is_timeout = False

        # Trạng thái đạt mục tiêu
        self.reached_goal = False

        self.smoothing_factor = 0.9  # Hệ số làm mượt cho vận tốc
    ...
    def check_crash(self, end_point):
        ...
        if x < 0 or x >= self.map.get_width() or y < 0 or y >= self.map.get_height():
            self.crash = True
            self.cost_function = (self.cost_function / self.it) + 400
            return True
...
if __name__ == "__main__":
    ...
    # start position
    start = (275, 25, np.pi/2)
    end_point = (328, 680, np.pi/2)
    running = True
    run_time = 0
    ...
    # enviroment
    env = Enviroment('C:/Users/admin/Desktop/HKII-24-25/CD_Robot_n_AI/Coding/hard-maze-2.jpg', end_point)
    ...
    # Số tham số cần tối ưu bằng PSO (bao gồm bias)
    npar = (n_inputs * n_hidden) + n_hidden + (n_hidden * n_outputs)
    ...
    Pbest_fitness = 999999*np.ones(pop_size)
    ...
    try:
        while running and current_iteration < max_iteration:
            ...
            for i in range(num_robot):
                robots.append(Robot(start, 'C:/Users/admin/Desktop/HKII-24-25/CD_Robot_n_AI/Coding/robotPenguin.png', env.map_copy))
            ...
            while num_robot_available > 0 and run_time < max_run_time:
                for idx, robot in enumerate(robots):
                    if not (robot.crash or robot.is_circling or robot.no_progress or robot.is_timeout or robot.reached_goal):
                        ...
                        if min_dist_to_visited > 10:  # Giảm ngưỡng
                            breakthrough_reward = -900  # Tăng phần thưởng
                        ...
                        if current_pos in robot.dead_end_positions:
                            dead_end_penalty = 300  # Giảm phạt

                        # Thưởng khi rất gần đích
                        proximity_bonus = 0
                        ...
                        if 10 < distance_to_goal < 50:  # Nếu robot ở gần đích nhưng không phải đích
                            proximity_bonus += -250  # Thêm phần thưởng nhỏ

                        # Tính khoảng cách từ robot đến điểm xuất phát
                        distance_to_start = np.sqrt((robot.x - start[0])**2 + (robot.y - start[1])**2)

                        # Áp dụng hình phạt nếu robot ở quá gần điểm xuất phát
                        start_proximity_penalty = 0
                        if distance_to_start < 25:  # Ngưỡng khoảng cách, có thể điều chỉnh
                            start_proximity_penalty = 300 / (distance_to_start + 1)  # Hình phạt tăng khi khoảng cách nhỏ

                        # Tổng hợp cost function
                        robot.cost_function = (0.5 * distance_to_goal +
                                              1.0 * distance_reward +
                                              0.02 * orientation_error +
                                              0.05 * wall_proximity + # tránh tường 0.05 -> 0.07
                                              0.01 * step_penalty +
                                              2.1 * breakthrough_reward + # 2.0 -> 2.1
                                              1.0 * movement_reward + 
                                              1.0 * dead_end_penalty +
                                              1.0 * proximity_bonus +
                                              1.0 * start_proximity_penalty)
                        ...
                        b_hidden = P_pso[idx, (n_inputs * n_hidden):(n_inputs * n_hidden + n_hidden)]
                        V = P_pso[idx, (n_inputs * n_hidden + n_hidden):].reshape(n_hidden, n_outputs)
                        VVV = vd_nn(X_nn, W, V, b_hidden)
                        v1, v2 = VVV.flatten()
                        
                        # Điều chỉnh vận tốc động
                        min_distance = min(robot.distances)
                        speed_factor = 0.5 if min_distance > 20 else min_distance / 40  # Giảm tốc độ khi gần tường
                        # robot.v1 = v1 * 2 * speed_factor
                        # robot.v2 = v2 * 2 * speed_factor
                        robot.v1 = robot.smoothing_factor * robot.v1 + (1 - robot.smoothing_factor) * (v1 * 2 * speed_factor)
                        robot.v2 = robot.smoothing_factor * robot.v2 + (1 - robot.smoothing_factor) * (v2 * 2 * speed_factor)
                        
                        # Giới hạn vận tốc tối đa
                        max_speed = 2.5  # Tùy chỉnh giá trị phù hợp
                        robot.v1 = max(-max_speed, min(max_speed, robot.v1))
                        robot.v2 = max(-max_speed, min(max_speed, robot.v2))
                        
                        # Thêm nhiễu ngẫu nhiên vào vận tốc
                        noise_factor = 0.2  # Hệ số nhiễu
                        robot.v1 += np.random.uniform(-noise_factor, noise_factor)
                        robot.v2 += np.random.uniform(-noise_factor, noise_factor)
                        ...
                        if robot.no_progress_count > 10:  # Nếu robot không tiến triển trong 10 bước
                            robot.v1 = np.random.uniform(-2, 2)  # Đặt lại vận tốc ngẫu nhiên
                            robot.v2 = np.random.uniform(-2, 2)
                            robot.no_progress_count = 0  # Đặt lại bộ đếm
